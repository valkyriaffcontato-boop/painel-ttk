...
// COLOQUE SUA CHAVE PIX ENTRE AS ASPAS ABAIXO:
const CHAVE_PIX_MANUAL = "eb3c68aa-c8f7-454c-b33d-b6e68a2a7281"; 

const WHATSAPP_LINK = "https://wa.me/5533991160044"; 
        ...
        if (interaction.isChatInputCommand()) {
            ...
            if (interaction.commandName === 'ticket') {
                const embed = new EmbedBuilder()
                    ...
                    .setDescription('🛠️ **SUPORTE:** Dúvidas ou Erros.\n👑 **ADMIN:** Falar com o Biel.')
                    .setColor('#3B82F6');
        ...
        if (interaction.isStringSelectMenu()) {
            if (interaction.customId === 'menu_compras') {
                ...
                const embedCompra = new EmbedBuilder()
                    .setTitle('🛒 PEDIDO - BIEL STORE')
                    .setDescription(`📦 Produto: **${item.label}**\n💰 Valor: **R$ ${item.price}**\n\n**CHAVE PIX:**\n\`${CHAVE_PIX_MANUAL}\`\n\nEnvie o comprovante e seu ID aqui.`)
                    .setColor('#22C55E');

                await channel.send({ content: `<@&${ID_CARGO_SUPORTE}> | <@${ID_BIEL_ADMIN}> | ${interaction.user}`, embeds: [embedCompra], components: [new ActionRowBuilder().addComponents(new ButtonBuilder().setCustomId('fechar_ticket').setLabel('Fechar').setStyle(ButtonStyle.Danger))] });
                return await interaction.reply({ content: `✅ Canal criado: ${channel}`, flags: [MessageFlags.Ephemeral] });
            }
            ...
            if (interaction.customId === 'tipo_ticket') {
                ...
                await channel.send({ content: `<@&${ID_CARGO_SUPORTE}> | ${interaction.user}`, embeds: [new EmbedBuilder().setTitle('Atendimento').setDescription('Como podemos ajudar?').setColor('#3B82F6')], components: [new ActionRowBuilder().addComponents(new ButtonBuilder().setCustomId('fechar_ticket').setLabel('Fechar').setStyle(ButtonStyle.Danger))] });
